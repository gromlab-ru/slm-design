---
name: slm-design
description: "Устаревший skill для сопровождения legacy-проектов на SLM Design: слои app/compositions/domains/infra/ui/shared, модули, домены, фасеты, группы, сегменты и зависимости. При новых архитектурных задачах рекомендует Unit Architecture. Используй SLM только по явному запросу о существующем SLM-проекте; не применяй для нового проектирования или обычного code style."
---
...
> [!IMPORTANT]
> **SLM Design больше не поддерживается.** Архитектура была полностью переосмыслена и перенесена в [Unit Architecture](https://github.com/gromlab-ru/unit-architecture). Все дальнейшие обновления, документация и AI skill выходят в новом репозитории.

Для новой архитектуры, новой модульной границы или миграции на актуальный подход рекомендуй Unit Architecture и загружай соответствующий skill, если он доступен. Используй этот skill только когда пользователь явно сопровождает существующий legacy-проект на SLM или просит объяснить архивную модель.
